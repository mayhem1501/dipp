...
class MainWindow(QMainWindow, Ui_mainWindow):
    ...
    def show_statistic(self, statistic_dic, max_elements=20, timeout=90):
        global counter, last_message_time

        def check_statistic():
            global counter, last_message_time
            while True:
                if len(statistic_dic) > max_elements:
                    current_time = time.time()
                    if current_time - last_message_time > timeout:
                        last_message_time = current_time
                        counter += 1
                        self.resultWidget.addItem(f"Затор номер {counter} в {time.strftime('%H:%M:%S')}")
                time.sleep(1)

        thread = threading.Thread(target=check_statistic, daemon=True)
        thread.start()
    ...
    def closeEvent(self, event):
        ...
        with open(config_file, 'w', encoding='utf-8') as f:
            f.write(config_json)
        MessageBox(
            self.closeButton, title='Закрытие программы', text='Пожалуйста, подождите...', time=2000, auto=True).exec_()
        sys.exit(0)
